...
export default {
  ...
  methods: {
    async obtenerCitas() {
      try {
        const response = await axios.get('https://api-production-edcf.up.railway.app/api/citas');
    ...
    async guardarCita() {
      try {
        if (this.isEditing) {
          await axios.put(`https://api-production-edcf.up.railway.app/api/citas/${this.cita.id}`, this.cita);
          this.mensaje = 'Cita actualizada exitosamente';
        } else {
          const response = await axios.post('https://api-production-edcf.up.railway.app/api/citas/programar', this.cita);
