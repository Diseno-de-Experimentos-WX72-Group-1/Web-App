    ...
    methods: {
      ...
      async guardarUsuario() {
        try {
          if (this.isEditing) {
            await axios.put(
              `https://petcare-fcaze8atc5cpdte0.canadacentral-01.azurewebsites.net/${this.usuario.id}`,
              this.usuario
            );
            this.mensaje = "Usuario actualizado exitosamente";
          } else {
            const response = await axios.post(
              "https://petcare-fcaze8atc5cpdte0.canadacentral-01.azurewebsites.net/",
              this.usuario
            );
      ...
      async eliminarUsuario(id) {
        try {
          await axios.delete(`https://petcare-fcaze8atc5cpdte0.canadacentral-01.azurewebsites.net/${id}`);
